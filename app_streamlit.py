...
def compact_rows(dataframe: pd.DataFrame, person: str, movement_type: str, limit: int = 5) -> str:
    rows_df = dataframe[(dataframe["person"] == person) & (dataframe["type_normalized"] == movement_type)].head(limit)
    if rows_df.empty:
        return '<div class="compact-empty">Sem movimentos para mostrar.</div>'
    ...
    for _, row in rows_df.iterrows():
        if movement_type == "salário":
            ...
            extra = f" · {description}" if str(row["category"]).lower() == "outros" and description else ""
            title = str(row["category"])
            meta = f"Despesa · {row['date']}{extra}"
        rows.append(
            f'''<div class="compact-row">
                <div class="compact-main">
                    <div class="compact-title">{escape(title)}</div>
                    <div class="compact-meta">{escape(meta)}</div>
                </div>
                <div class="compact-value {value_class}">{money(row['value'])}</div>
            </div>'''
        )
    return "".join(rows)


def person_movement_panels(title: str, dataframe: pd.DataFrame, movement_type: str) -> None:
    section_title(title)
    panels = []
    for person in PEOPLE:
        rows = compact_rows(dataframe, person, movement_type)
        panels.append(
            f'''<div class="compact-panel">
                <div class="compact-panel-title"><strong>{person}</strong><span>{title}</span></div>
                {rows}
            </div>'''
        )
    st.markdown(f'<div class="compact-grid">{"".join(panels)}</div>', unsafe_allow_html=True)

def render_dashboard(filtered_df: pd.DataFrame) -> None:
    page_title("Casal", "Visão geral conjunta das vossas contas.")
        ...
        st.markdown(
            ...
            <div class="movement-card person-summary-card">
                <div class="movement-top">
                    ...
                    <div class="{balance_class(balance)}">{money(balance)}</div>
                </div>
            </div>
            """,
    ...
    person_movement_panels("Receitas por pessoa", filtered_df, "salário")
    person_movement_panels("Despesas por pessoa", filtered_df, "despesa")
